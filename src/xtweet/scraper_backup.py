...
class ContentScraper:
    ...
    def _extract_content(self, target: ScrapingTarget) -> List[Dict[str, Any]]:
        ...
        if config.get('type') == 'rss':
            return self._extract_rss_content(target)
        elif self.use_selenium:
            return self._extract_with_selenium(target)
        else:
            return self._extract_with_requests(target)
    
    def _extract_rss_content(self, target: ScrapingTarget) -> List[Dict[str, Any]]:
        """RSS 피드에서 콘텐츠 추출
        ...
        """
        try:
            logger.info(f"RSS 피드 파싱 시작: {target.url}")
            
            # feedparser로 RSS 피드 파싱
            feed = feedparser.parse(target.url)
            
            if not feed.entries:
                logger.warning(f"RSS 피드에서 항목을 찾을 수 없음: {target.url}")
                return []
            ...
            # 테스트 모드에서는 제한된 수만 처리
            if self.test_mode:
                entries = entries[:TEST_MODE_ARTICLE_LIMIT]
                logger.info(f"테스트 모드: 최대 {TEST_MODE_ARTICLE_LIMIT}개 항목만 처리")
            
            for entry in entries:
                content_data = self._extract_rss_entry_data(entry, target)
                if content_data and self._is_valid_content(content_data):
                    if self._is_recent_content(content_data):
                        contents.append(content_data)
                    else:
                        logger.debug(f"시간 필터링으로 제외: {content_data.get('title', '')[:50]}...")
            
            logger.info(f"RSS 피드 파싱 완료: {len(contents)}개 유효 항목")
            return contents
            
        except Exception as e:
            logger.error(f"RSS 피드 파싱 실패: {target.url}, 오류: {str(e)}")
            raise
    
    def _extract_rss_entry_data(self, entry, target: ScrapingTarget) -> Optional[Dict[str, Any]]:
        """RSS 항목에서 데이터 추출
        ...
        try:
            ...
            # 개별 기사 페이지에서 본문 추출 시도
            full_content = ''
            if link and link.startswith('http'):
                full_content = self._extract_article_content(link)
            
            # 본문이 없으면 요약 사용
            content = full_content if full_content else summary
            
            # 발행 시간 추출
            published_time = self._parse_rss_published_time(entry)
            ...
            logger.warning(f"RSS 항목 데이터 추출 실패: {str(e)}")
            return None
    
    def _parse_rss_published_time(self, entry) -> Optional[datetime]:
        """RSS 항목에서 발행 시간 추출
        ...
        try:
            ...
            if hasattr(entry, 'published_parsed') and entry.published_parsed:
                ...
                return datetime.fromtimestamp(time_module.mktime(entry.published_parsed))
            
            # 수동 파싱 시도
            published_str = entry.get('published', '') or entry.get('pubDate', '')
            if published_str:
                return self._parse_datetime_string(published_str)
                
        except Exception as e:
            logger.debug(f"RSS 발행 시간 파싱 실패: {str(e)}")
