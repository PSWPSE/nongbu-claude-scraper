...
def api_generate_content():
    ...
    try:
        ...
        # 트위터 최적화 간결한 금융 뉴스 프롬프트
        prompt = f"""당신은 전문 금융, 투자, 경제 기자입니다. 다음 금융 뉴스를 바탕으로 트위터 게시에 최적화된 간결하고 임팩트 있는 콘텐츠를 작성해주세요.
...
5. **과장 없는 객관적 분석** - 사실 기반 정보
6. **트위터 게시 최적화 형태** - 짧고 임팩트 있는 문장
7. **섹션 구분선은 '-' 10개 길이만 사용** - 예: ----------
8. **표준화된 구성 사용 금지** - 자유로운 형식으로 작성
9. **모든 내용을 한국어로 작성** - 읽기 쉬운 언어

위 지침을 준수하여 핵심만 담은 임팩트 있는 콘텐츠를 작성하세요."""
...
*본 콘텐츠는 NongBu AI 시스템에 의해 자동 생성되었습니다.*
"""
